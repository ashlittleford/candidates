...
def create_htaccess():
    ...
    # In cPanel/CloudLinux, the virtualenv python is usually in a path containing 'virtualenv'
    # and sys.prefix != sys.base_prefix
    in_venv = (sys.prefix != sys.base_prefix) or ('virtualenv' in python_path)

    if not in_venv:
        print("\n" + "!"*70)
        print("WARNING: You are seemingly NOT running inside a virtual environment.")
        print(f"Current Python Interpreter: {python_path}")
        print("!"*70)
        print("If you proceed, the generated .htaccess will point to the system Python,")
        print("which likely lacks the necessary dependencies (Flask, etc).")
        print("")
        print("We STRONGLY recommend activating your virtual environment first.")
        print("Command usually looks like: source /home/user/virtualenv/.../bin/activate")
        print("")
        confirm = input("Are you sure you want to proceed with this Python path? [y/N]: ").strip().lower()
        if confirm != 'y':
            print("Aborted. Please activate your virtual environment and try again.")
            sys.exit(1)
